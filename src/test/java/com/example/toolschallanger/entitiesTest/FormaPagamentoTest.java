...
public class FormaPagamentoTest {
    ...
    public void deveValidaParcelaSucesso() {
        ...
        Assertions.assertDoesNotThrow(() -> transacaoModel.getFormaPagamentoModel().validaParcela(transacaoModel));
    }

    @Test
    public void deveValidaParcelaErro() {
        TransacaoModel transacaoModel = new TransacaoModel(UUID.randomUUID(), 1065151L, new DescricaoModel(500.00, LocalDateTime.parse("2021-01-01T18:30:00"), "PetShop", 0000.1111, 00000.000, Status.AUTORIZADO), new FormaPagamentoModel(PARCELADO_EMISSOR, 1));
        Assertions.assertNotNull(transacaoModel);
        Assertions.assertThrows(RuntimeException.class, () -> transacaoModel.getFormaPagamentoModel().validaParcela(transacaoModel));
    }
